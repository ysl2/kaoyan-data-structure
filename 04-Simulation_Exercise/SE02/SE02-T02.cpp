...
BiTNode *getParentNode(BiTree T, BiTNode *p) {
    if (T == NULL)
        return NULL;
    if (T->lchild == p || T->rchild == p)
        return T;
    BiTNode *result;
    if ((result = getParentNode(T->lchild, p)) != NULL)
        return result;
    return getParentNode(T->rchild, p);
}
...
void test(ElemType *preOrder, ElemType *inOrder, int length, ElemType x) {
    ...
    cout << endl;
    BiTNode *result;
    getNodeByValue(T, x, result);
    cout << getParentNode(T, result)->data;
